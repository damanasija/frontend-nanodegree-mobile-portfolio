...
var sizeSwitcher = function(size) {
  switch(size) {
    case "1":
      return 0.25;
    case "2":
...
var determineDx = function(elem, size) {
...
var changePizzaSizes = function(size) {
  ...
  //calculating once for all pizzas
...
function updatePositions() {
  ...
  var scroll = lastScrollY / 1250;
  for(var i = 0; i < items.length; i++) {
    var phase = Math.sin(scroll + (i % 5));
    //using transform
    var prop = 'translateX(' + 100 * phase + 'px)';
...
document.addEventListener('DOMContentLoaded', function() {
  ...
  var cols = Math.floor(visWidth / s);
  var rows = Math.floor(visHeight / s);
